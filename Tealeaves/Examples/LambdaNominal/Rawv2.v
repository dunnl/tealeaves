...
Proof.
  refine (fix rename l x y t (Ht: Acc term_lt t) {struct Ht} :=
    ...
    | lam b t' =>
        fun phi =>
          if b == x
          then (@exist _ _ (lam b t') _)
          else let z := fresh l in
...
Section rw.
  ...
  Proof.
    ...
    - simpl.
      ...
      { destruct (term_lt_wf (lam b t)).
        ...
        pose (sig_eta (rename' (l ++ [fresh l]) b (fresh l) t (term_lt_wf t))).
        rewrite e.
        clear e.
        rewrite normalize_rename'.
        pose (sig_eta
                (rename' (l ++ [fresh l]) x y (proj1_sig (rename' (l ++ [fresh l]) b (fresh l) t (term_lt_wf t)))
                   (term_lt_wf (proj1_sig (rename' (l ++ [fresh l]) b (fresh l) t (term_lt_wf t)))))).
        rewrite e.
        reflexivity.
        }
    - simpl.
  ...
  Lemma rename_rw2_neq: forall l x y b t,
      ...
      b = y ->
      rename l x y (lam b t) =
        (λ) (fresh l) (rename (l ++ [fresh l]) x y (rename (l ++ [fresh l]) y (fresh l) t)).
  Proof.
    intros.
    rewrite rename_eq_iter.
    unfold rename_iter.
