...
Section rw_mbinddt_type.
  ...
  Lemma rw_mbinddt_type1 : forall c, mbinddt typ F f (ty_c c) = pure (ty_c c).
  Proof. reflexivity. Qed.

  Lemma rw_mbinddt_type2 : forall (a : A), mbinddt typ F f (ty_v a) = f KType (nil, a).
  Proof. reflexivity. Qed.

  Lemma rw_mbinddt_type3 : forall (t1 t2 : typ A), mbinddt typ F f (ty_ar t1 t2) = pure ty_ar <⋆> mbinddt typ F f t1 <⋆> mbinddt typ F f t2.
  Proof. reflexivity. Qed.

  Lemma rw_mbinddt_type4 : forall (body : typ A), mbinddt typ F f (ty_univ body) = pure ty_univ <⋆> mbinddt typ F (fun k => f k ∘ incr [KType]) body.
  Proof. reflexivity. Qed.
...
Section rw_mmapdt_type.
  ...
  Lemma rw_mmapdt_type1 : forall c, mmapdt typ F f (ty_c c) = pure (ty_c c).
  Proof. reflexivity. Qed.

  Lemma rw_mmapdt_type2 : forall (a : A), mmapdt typ F f (ty_v a) = pure ty_v <⋆> (f KType (nil, a)).
  Proof.
    intros. rewrite <- map_to_ap.
    unfold mmapdt. now rewrite rw_mbinddt_type2.
  Qed.

  Lemma rw_mmapdt_type3 : forall (t1 t2 : typ A), mmapdt typ F f (ty_ar t1 t2) = pure (ty_ar) <⋆> (mmapdt typ F f t1) <⋆> (mmapdt typ F f t2).
  Proof. reflexivity. Qed.

  Lemma rw_mmapdt_type4 : forall (body : typ A), mmapdt typ F f (ty_univ body) = pure (ty_univ) <⋆> (mmapdt typ F (fun k => f k ∘ incr [KType]) body).
  Proof. reflexivity. Qed.
...
Section rw_tomlistd_type.
  ...
  Lemma rw_tomlistd_type4 : forall (body : typ A), tomlistd typ (ty_univ body) = map (F := list) (incr [KType]) (tomlistd typ body).
  Proof.
    intros. unfold tomlistd, tomlistd_gen.
    rewrite rw_mmapdt_type4. cbn.
    compose near body on right. unfold mmapdt.
    rewrite (dtp_mbinddt_morphism
               (list (@K I2)) typ SystemF (const (list _)) (const (list _)) (ϕ := (fun _ => map (F := list) (incr [KType])))
...
Section rw_mbinddt_term.
  ...
  Lemma rw_mbinddt_term2 : forall (τ : typ A) (t : term A), mbinddt term F f (tm_abs τ t) = pure tm_abs <⋆> (mbinddt typ F f τ) <⋆> (mbinddt term F (fun k => f k ∘ incr [KTerm]) t).
  Proof. reflexivity. Qed.

  Lemma rw_mbinddt_term3 : forall (t1 t2 : term A), mbinddt term F f (tm_app t1 t2) = pure tm_app <⋆> (mbinddt term F f t1) <⋆> (mbinddt term F f t2).
  Proof. reflexivity. Qed.

  Lemma rw_mbinddt_term4 : forall (t : term A), mbinddt term F f (tm_tab t) = pure tm_tab <⋆> (mbinddt term F (fun k => f k ∘ incr [KType]) t).
  Proof. reflexivity. Qed.

  Lemma rw_mbinddt_term5 : forall (t: term A) (τ : typ A), mbinddt term F f (tm_tap t τ) = pure tm_tap <⋆> (mbinddt term F f t) <⋆> (mbinddt typ F f τ).
  Proof. reflexivity. Qed.
