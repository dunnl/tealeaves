...
Section sec.
  ...
  (*
    Lemma multi_preincr_assoc: forall w,
        (g ◻ f) ◻ (allK (incr w)) =
          (g ◻ f) ◻ allK (incr w).
   *)
...
Section rw_tolistmd_type.
  ...
  Proof.
    ...
    rewrite (dtp_mbinddt_morphism
               (list (@K I2)) term SystemF (allK (list (list K2 * (K * A))))
               (allK (list _)) (ϕ:= (fun _ => map (F:= list) (incr [ktrm])))
               (fun (k: @K I2) => map (F:= allK (list (list K2 * (K * A))))
  ...
  Proof.
    ...
    compose near t on right. unfold mmapdt.
    rewrite (dtp_mbinddt_morphism
               (list (@K I2)) term SystemF (allK (list _)) (allK (list _)) (ϕ:= (fun _ => map (F:= list) (incr [ktyp])))
               (fun (k: @K I2) => map (F:= allK (list (list K2 * (K2 * A)))) (mret SystemF k) ∘ (fun '(w, a) => [(w, (k, a))]))).
    fequal. now ext k [w a].
  Qed.
