...
Section induction_principle.
  ...
  Context
    (var : Type)
    (P : term var -> Prop)
    (case_var : forall v : var, P (@tvar var v))
    (case_app : forall (t1 t2 : term var), P t1 -> P t2 -> P (app var t1 t2))
    (case_lam : forall (a : atom) (t : term var), P t -> P (lam var a t))
    (case_let :
      forall (binders : (atom * term var))
        (IHrec : forall (bound : term var), bound ∈ binders -> P bound)
        (t : term var),
        P t ->
        P (letin var binders t)).

  Definition term_mind : forall (t : term var), P t.
    refine(
    fix F (t : term var) : P t :=
      match t return (P t) with
      | tvar _ y => case_var y
      | app _ t1 t2 => case_app t1 t2 (F t1) (F t2)
      | lam _ nm body => case_lam nm body (F body)
      | letin _ (nm, def) body =>
          @case_let (nm, def) (fun (bound : term var)
                                 (elem : (bound ∈ (nm, def)))
                                 => _ (F def))
            body (F body)
      end).
...
Section induction_principle.
  ...
  Context
    ...
    (case_lam : forall (a : atom) (t : term var), P t -> P (lam var a t))
    (case_let :
      forall (a : atom) (def : term var) (body : term var)
        (IHdef : P def)
        (IHbody : P body),
        P (letin _ (a, def) body)).

  Definition term_mind2 : forall (t : term var), P t.
    refine(
    fix F (t : term var) : P t :=
      match t return (P t) with
      | tvar _ y => case_var y
      ...
      | lam _ nm body => case_lam nm body (F body)
      | letin _ (nm, def) body =>
          @case_let nm def body (F def) (F body)
      end).
  Defined.
...
Fixpoint binddt_term (G : Type -> Type) `{Fmap G} `{Pure G} `{Mult G}
  ...
  | letin _ p body =>
      pure G (@letin v2) <⋆> traverse (atom ×) G (binddt_term G f) p <⋆> binddt_term G (fun k => f k ∘ incr [fst p]) body
  end.
...
Section laws.
  ...
  Ltac induction_on_term :=
    match goal with
    | t : term ?v |- _ => induction t using term_mind2
    end.
...
|*)
  ...
  Proof.
    intros. ext t. unfold id. induction t using term_mind2.
    - cbn. reflexivity.
    - cbn. fequal.
      + assumption.
      + assumption.
...
|*)
  ...
  Proof.
    ...
    induction t using term_mind2; intros f g.
    ...
    - cbn.
      ...
      unfold_ops @Pure_compose.
      (* right side, first argument *)
      change (fmap (F ∘ G) ?f) with (fmap F (fmap G f)).
      rewrite_strat innermost (terms (ap_compose2 G F)).
      rewrite (app_pure_natural F).
      rewrite <- ap_fmap.
      rewrite (app_pure_natural F).
...
|*)
  Section law4.
    ...
    Proof.
      intros. ext t. unfold compose at 1.
      generalize dependent f.
      induction t using term_mind2; cbn; intro f.
      - reflexivity.
      - (* left side *)
      ...
      - (* left side *)
        repeat rewrite ap_morphism_1.
        rewrite (appmor_pure F G).
        invert_applicative; rewrite appmor_natural.
        (* right side *)
        rewrite <- IHt1.
        rewrite <- IHt2.
        (* Done *)
