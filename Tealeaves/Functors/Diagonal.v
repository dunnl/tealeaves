...
(******************************************************************************)
#[export] Instance Pure_Diagonal : Pure Diagonal :=
  fun A a => (a, a).

#[export] Instance Mult_Diagonal : Mult Diagonal :=
  fun A B (x : Diagonal A * Diagonal B) => ((fst (fst x), fst (snd x)), (snd (fst x), snd (snd x))).
...
Lemma traverse_Diagonal_rw {A B: Type} {G} `{Map G} `{Mult G} `{Pure G} (f: A -> G B) (a1 a2: A):
  traverse (T := fun A => A * A) f (a1, a2) = pure pair <⋆> f a1 <⋆> f a2.
Proof.
  reflexivity.
Qed.

#[export] Instance TraversableFunctor_Diagonal: TraversableFunctor (fun A => A * A).
Proof.
  constructor.
