...
#[export] Instance Bind_set : Bind set set := fun A B f s_a =>
(fun b => exists (a : A), s_a a /\ f a b).

#[local] Notation "{{ x }}" := (ret set _ x).
...
Lemma bind_set_one `{f : A -> set B} {a : A} :
  bind set set A B f {{ a }} = f a.
Proof.
  solve_basic_set.
Qed.

Lemma bind_set_add `{f : A -> set B} {x y} :
  bind set set A B f (x ∪ y) = bind set set A B f x ∪ bind set set A B f y.
Proof.
  solve_basic_set.
Qed.
...
Lemma set_bind0 : forall (A B : Type) (f : A -> set B),
    bind set set A B f ∘ ret set A = f.
Proof.
  intros. ext a. unfold compose.
  now autorewrite with tea_set.
Qed.

Lemma set_bind1 : forall A : Type, bind set set A A (ret set A) = id.
  intros. cbv. ext s a. propext.
  - intros [a' [h1 h2]]. now subst.
  - intro. eexists a. intuition.
Qed.

Lemma set_bind2 : forall (A B C : Type) (g : B -> set C) (f : A -> set B),
    bind set set B C g ∘ bind set set A B f = bind set set A C (g ⋆1 f).
Proof.
  intros. ext a. unfold compose.
  cbv. ext c. propext.
