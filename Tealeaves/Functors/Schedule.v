...
Section Schedule.
  ...
  Fixpoint fmap_Schedule `{f : C -> D} `(c : Schedule C) : Schedule D :=
    ...
    | Go _ c => Go D (f c)
    | Ap _ rest (pair w a) =>
      Ap D (@fmap_Schedule (T B -> C) (T B -> D) (compose f) rest) (w, a)
    end.
  ...
  Fixpoint mult_Schedule `(jc : Schedule C) `(jd : Schedule D) : Schedule (C * D) :=
    ...
    | Ap _ rest (pair w a) =>
        Ap (C * D) (fmap Schedule strength_arr (mult_Schedule jc rest)) (pair w a)
    end.
  ...
  Lemma mult_Schedule_rw4 : forall (w : W) (a : A) `(jc : @Schedule (T B -> C)) `(d : D),
      (jc ⧆ (w, a)) ⊗ Go D d = fmap Schedule (costrength_arr ∘ pair_right d) jc ⧆ (w, a).
  Proof.
    easy.
  Qed.

  Lemma mult_Schedule_rw5 : forall (w : W) (a : A) `(jc : @Schedule (T B -> C)) `(d : D),
      Go D d ⊗ (jc ⧆ (w, a)) = fmap Schedule (strength_arr ∘ pair d) jc ⧆ (w, a).
  Proof.
...
Arguments Ap {T}%function_scope {W A B}%type_scope [C]%type_scope _ _.
...
Section functoriality_Schedule.
  ...
  Fixpoint mapfst_Schedule {A1 A2 B C} (f : A1 -> A2) `(j : @Schedule T W A1 B C) : @Schedule T W A2 B C :=
    match j with
    | Go _ a => Go _ a
...
Fixpoint runSchedule
         ...
         (ϕ : W * A -> F (T B))
         `(j : @Schedule T W A B C) : F C :=
  match j with
  | Go _ a => pure F a
  | @Ap _ _ _ _ _ rest (pair w a) => runSchedule T F ϕ rest <⋆> ϕ (w, a)
  end.
...
Section runSchedule_rw.
  ...
  Lemma runSchedule_rw1 (c : C) :
    runSchedule T F f (Go _ c) = pure F c.
  Proof.
    reflexivity.
  Qed.

  Lemma runSchedule_rw2 (w : W) (a : A) (rest : Schedule (T B -> C)) :
    runSchedule T F  f (rest ⧆ (w, a)) = runSchedule T F  f rest <⋆> f (w, a).
  Proof.
    reflexivity.
  Qed.
...
Section runSchedule_naturality.
  ...
  Lemma natural_runSchedule (f : C -> D) (j : @Schedule T W A B C) :
    fmap F f (runSchedule T F  ϕ j) = runSchedule T F  ϕ (fmap Schedule f j).
  Proof.
...
Section runSchedule_morphism.
  ...
  Lemma appmor_pure_runSchedule : forall (C : Type) (c : C),
      runSchedule T F  f (pure Schedule c) = pure F c.
  Proof.
    easy.
  Qed.

  Lemma appmor_mult_runSchedule : forall (C D : Type) (x : Schedule C) (y : Schedule D),
      runSchedule T F  f (x ⊗ y) = runSchedule T F  f x ⊗ runSchedule T F  f y.
  Proof.
    ...
    - intros. destruct p. rewrite runSchedule_rw2.
      ...
      compose near (runSchedule T F f (x ⊗ y) ⊗ f (w, a)).
      rewrite (fun_fmap_fmap F).
      cbn. unfold ap. change (mult_Schedule ?jx ?jy) with (jx ⊗ jy).
      rewrite <- natural_runSchedule; auto.
      rewrite (app_mult_natural_l F).
      compose near (runSchedule T F f (x ⊗ y) ⊗ f (w, a)) on left.
      rewrite (fun_fmap_fmap F). fequal. now ext [[? ?] ?].
  Qed.

  #[export] Instance Morphism_store_fold: ApplicativeMorphism Schedule F (@runSchedule T W A B F _ _ _ f).
  Proof.
...
Section runSchedule_morphism.
  ...
  Lemma runSchedule_morphism `(j : @Schedule T W A B C) :
    @ψ C (runSchedule T F f j) = runSchedule T G (@ψ (T B) ∘ f) j.
  Proof.
    induction j.
