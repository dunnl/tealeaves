...
#[export] Instance Map_Batch2 {A1 A2 B1 B2: Type}:
  Map (Batch2 A1 A2 B1 B2) :=
...
(** *** Operations *)
(**********************************************************************)
#[export] Instance Pure_Batch2 {A1 A2 B1 B2}:
  Pure (@Batch2 A1 A2 B1 B2) :=
  @Done2 A1 A2 B1 B2.
...
#[program] Fixpoint mult_Batch {A1 A2 B1 B2} {C1 C2: Type}
  ...
  match b2 with
  | Done2 c2 =>
      map (F := Batch2 A1 A2 B1 B2) (fun (c1: C1) => (c1, c2)) b1
  | StepA mk a =>
      StepA
        (map (F := Batch2 A1 A2 B1 B2)
