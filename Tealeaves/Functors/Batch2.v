...
Section Batch2.
  ...
  Fixpoint fmap_Batch2 `{f : C -> D} `(c : Batch2 C) : Batch2 D :=
    match c with
    | Done _ c => Done D (f c)
    | Step _ rest a =>
      Step D (@fmap_Batch2 (T B -> C) (T B -> D) (compose f) rest) a
    end.

  #[export] Instance Fmap_Batch2 : Fmap Batch2 := @fmap_Batch2.

  Lemma fmap_id_Batch2 : forall (C : Type),
      fmap Batch2 id = @id (Batch2 C).
  Proof.
  ...
  Lemma fmap_fmap_Batch2 : forall (C D E : Type) (f : C -> D) (g : D -> E),
      fmap Batch2 g ∘ fmap Batch2 f = fmap Batch2 (g ∘ f).
  Proof.
  ...
  #[export] Instance Functor_Batch2 : Functor Batch2 :=
    {| fun_fmap_id := fmap_id_Batch2;
       fun_fmap_fmap := fmap_fmap_Batch2;
    |}.
  ...
  Fixpoint mult_Batch2 `(jc : Batch2 C) `(jd : Batch2 D) : Batch2 (C * D) :=
    match jd with
    | Done _ d => fmap Batch2 (fun (c : C) => (c, d)) jc
    | Step _ rest a =>
        Step (C * D) (fmap Batch2 strength_arrow (mult_Batch2 jc rest)) a
    end.
  ...
  Proof.
    ...
    - cbn. repeat change (mult_Batch2 ?jx ?jy) with (jx ⊗ jy).
      ...
      do 2 rewrite (fun_fmap_fmap Batch2).
      compose near (x ⊗ y ⊗ rest) on right.
      rewrite (fun_fmap_fmap Batch2).
      fequal. now ext [[? ?] ?].
  Qed.

  Lemma app_unital_l_Batch2 : forall (A : Type) (x : Batch2 A),
      fmap Batch2 left_unitor (pure Batch2 tt ⊗ x) = x.
  Proof.
  ...
  Lemma app_unital_r_Batch2 : forall (A : Type) (x : Batch2 A),
      fmap Batch2 right_unitor (x ⊗ pure Batch2 tt) = x.
  Proof.
    intros. induction x as [ANY any | ANY rest IH a].
    - easy.
    - cbn in *. fequal. rewrite <- IH at 2.
      compose near rest. now do 2 rewrite (fun_fmap_fmap Batch2).
  Qed.
...
Section functoriality_Batch2.
  ...
  Fixpoint mapfst_Batch2 `{Fmap W} {A1 A2 B C} (f : A1 -> A2) `(j : @Batch2 W T A1 B C) : @Batch2 W T A2 B C :=
    match j with
    | Done _ a => Done _ a
    | Step rest p => Step (mapfst_Batch2 f rest) (fmap W f p)
    end.
...
Section runBatch2_naturality.
  ...
  Lemma natural_runBatch2 (f : C -> D) (j : @Batch2 W T A B C) :
    fmap F f (runBatch2 W T F ϕ j) = runBatch2 W T F ϕ (fmap Batch2 f j).
  Proof.
    generalize dependent D. induction j; intros.
    - cbn. now rewrite (app_pure_natural F).
    - cbn. rewrite fmap_ap. fequal.
      now rewrite IHj.
  Qed.
