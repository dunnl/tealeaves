...
(* Register <<Functor>> instance for Kleisli <<TraversableFunctor>> *)
...
Definition batch (A B : Type) : A -> Batch A B B :=
  Step (Done (@id B)).
  (* = fun a => (Done A B (@id B)) ⧆ a. *)

Definition toBatch (T : Type -> Type) (A B : Type) `{Traverse T} : T A -> Batch A B (T B) :=
  traverse T (Batch A B) A B (batch A B).
