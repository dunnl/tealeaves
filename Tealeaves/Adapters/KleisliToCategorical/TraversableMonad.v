...
Module ToCategorical.
  Section with_monad.
    ...
    Lemma mon_join_ret_T : forall A : Type, join ∘ ret (T := T) (A := T A) = @id (T A).
    Proof.
    ...
    Lemma trvmon_ret_T : forall (G : Type -> Type) (H3 : Map G) (H4 : Pure G) (H5 : Mult G),
        Applicative G -> forall A : Type, dist T G ∘ ret (T := T) (A := G A) = map (F := G) (ret (T := T)).
    Proof.
    ...
    Lemma trvmon_join_T : forall (G : Type -> Type) (H3 : Map G) (H4 : Pure G) (H5 : Mult G),
        Applicative G -> forall A : Type, dist T G ∘ join (T := T) (A := G A) =
