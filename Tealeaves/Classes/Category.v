(*|
  ...
  The implementation closely resembles that of `math-classes
  <https://github.com/coq-community/math-classes>`_ but this library
  focuses on more functor classes and is typically specialized to the
...
(**********************************************************************)
Module Notations.
  Infix "⟶" := (homset) (at level 90, right associativity):
      category_scope.
  Infix "⊙":= (comp _ _ _) (at level 40, left associativity):
      category_scope.
  Notation "F ⇒ G" := (forall a: _, homset (F a) (G a)) (at level 50):
      category_scope.
End Notations.
...
(** Endofunctors are closed under composition and identities. This
    gives monoidal structure on the category of endofunctors but we do
    not need to formalize that far. *)
(**********************************************************************)
...
Section monad_homomorphism.
  ...
  Class Monad_Hom (mhom: forall a, T a ⟶ U a) :=
    { mhom_domain: Monad T;
      ...
      mhom_join:
      `(mhom a ⊙ (join T a) =
          join U a ⊙ (mhom (U a) ⊙ (fmap T (mhom a))));
    }.
