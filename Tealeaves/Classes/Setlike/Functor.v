  ...
  Class SetlikeFunctor :=
    { xfun_natural :> Natural (@toset F _);
      ...
      xfun_respectful : forall A B (t : F A) (f g : A -> B),
        (forall a, a ∈ t -> f a = g a) -> map F A B f t = map F A B g t;
    }.
...
Section SetlikeFunctor_set_instance.
  ...
  Lemma toset_set_respectful : forall (A B : Type) (t : A -> Prop) (f g : A -> B),
      (forall a : A, a ∈ t -> f a = g a) -> map set A B f t = map set A B g t.
  Proof.
...
Section setlike_functor_theory.
  ...
  Theorem in_map_iff : forall t f b,
      b ∈ map F A B f t <-> exists a : A, a ∈ t /\ f a = b.
  Proof.
  ...
  Corollary in_map_mono : forall t f a,
      a ∈ t -> f a ∈ map F A B f t.
  Proof.
    introv. rewrite in_map_iff. now exists a.
  Qed.

  Corollary map_respectful : forall t f g,
      (forall a, a ∈ t -> f a = g a) -> map F A B f t = map F A B g t.
  Proof.
  ...
  Corollary map_respectful_id : forall t (f : A -> A),
      (forall a, a ∈ t -> f a = a) -> map F A A f t = t.
  Proof.
    intros. replace t with (map F A A id t) at 2
