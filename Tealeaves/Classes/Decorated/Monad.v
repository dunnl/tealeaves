...
Import Classes.Comonad.Notations.
...
#[local] Arguments bindd {M}%type_scope {T}%function_scope (U)%function_scope {Bindd} {A B}%type_scope _%function_scope _.
...
Module DerivedInstances.
  ...
  Section operations.
    ...
    Lemma bind_to_bindd `(f : A -> T B) :
      bind T T A B f = bindd T (f ∘ extract (W ×) A).
    Proof.
      reflexivity.
    Qed.

    Lemma mapd_to_bindd `(f : W * A -> B):
      mapd W T A B f = bindd T (ret T B ∘ f).
    Proof.
      reflexivity.
    Qed.

    Lemma map_to_bindd `(f : A -> B):
      map T f = bindd T (ret T B ∘ f ∘ extract (W ×) A).
    Proof.
      reflexivity.
    Qed.
  ...
  Section kc_special_cases.
    ...
    Lemma kc5_45 {A B C} : forall (g : W * B -> C) (f : W * A -> T B),
        (ret T C ∘ g) ⋆5 f = fun '(w, t) => mapd W T B C (g ⦿ w) (f (w, t)).
    Proof.
      reflexivity.
    Qed.
    ...
    Lemma kc5_41 {A B C} : forall (g : W * B -> C) (f : A -> T B),
        (ret T C ∘ g) ⋆5 (f ∘ extract (W ×) A) = (fun '(w, a) => mapd W T B C (g ⦿ w) (f a)).
    Proof.
      reflexivity.
    Qed.
  ...
  Section laws.
    ...
    Corollary bind_bindd {A B C} : forall (g : B -> T C) (f : W * A -> T B),
        bind T T B C g ∘ bindd T f = bindd T (g ⋆1 f).
    Proof.
      intros.
    ...
    Corollary bindd_bind {A B C} : forall (g : W * B -> T C) (f : A -> T B),
        bindd T g ∘ bind T T A B f = bindd T ((fun '(w, t) => bindd T (g ⦿ w) t) ∘ map (W ×) f).
    Proof.
      intros.
      rewrite (bind_to_bindd).
      rewrite (kmond_bindd2 T).
    ...
    Lemma bindd_mapd {A B C} : forall (g : W * B -> T C) (f : W * A -> B),
        bindd T g ∘ mapd W T A B f = bindd T (g ⋆4 f).
    Proof.
      intros.
    ...
    Corollary mapd_bindd {A B C} : forall (g : W * B -> C) (f : W * A -> T B),
        mapd W T B C g ∘ bindd T f = bindd T (fun '(w, t) => mapd W T B C (g ⦿ w) (f (w, t))).
    Proof.
      intros.
      rewrite (mapd_to_bindd).
      rewrite (kmond_bindd2 T).
    ...
    Lemma mapd_mapd : forall (A B C : Type) (g : W * B -> C) (f : W * A -> B),
        mapd W T B C g ∘ mapd W T A B f = mapd W T A C (g ⋆4 f).
    Proof.
      intros.
    ...
    Lemma mapd_bind : forall (A B C : Type) (g : W * B -> C) (f : A -> T B),
        mapd W T B C g ∘ bind T T A B f =
          bindd T (fun '(w, a) => mapd W T B C (g ⦿ w) (f a)).
    Proof.
      intros.
      rewrite (mapd_to_bindd).
      rewrite (bind_to_bindd).
    ...
    Lemma mapd_map : forall (A B C : Type) (g : W * B -> C) (f : A -> B),
        mapd W T B C g ∘ map T f =
          mapd W T A C (g ∘ map (W ×) f).
    Proof.
      intros.
      rewrite (mapd_to_bindd).
      rewrite (map_to_bindd).
  ...
  Section instances.
    ...
    Proof.
      constructor.
      - intros. unfold_ops @Bind_Bindd; now rewrite (kmond_bindd0 T).
      - intros. unfold_ops @Bind_Bindd; now rewrite (kmond_bindd1 T).
