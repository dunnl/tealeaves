...
Module Notations.
  ...
  #[global] Notation "A ~k~> G B" :=
    (forall k, A -> G k B%type)
      (at level 70, G at level 5, B at level 5): tealeaves_multi_scope.

  (** This notation is similar to but more general than <<⊙>> because
      it works even when <<g>> or <<f>> are dependently-typed (and
  ...
  #[global] Notation "F ⇒ G" := (forall A: Type, F A -> G A)
...
Section targeted_morphisms.
  ...
  (********************************************************************)
  (** We create some hint databases for rewriting expressions
      involving the "targeting" combinators defined in this section
      and elsewhere. These should typically be used with
      [autorewrite*] in order to ensure the correct lemmas are invoked
      (namely, those which do not raise side-conditions that cannot be
      solved, typically because a <<XXX_neq>> lemma has been chosen
      when <<XXX_eq>> is the right one). Due to a bug involving
      [autorewrite*] we also create smaller databases that are
      convenient to use with [autorewrite] (without the <<*>>), at the
      cost of verbosity. *)
  (** See https://github.com/coq/coq/issues/14344 *)
  ...
  (** Build a k-morphism that targets only the leaves belonging to a
      partition <<k>>. This must be restricted to morphisms that do
