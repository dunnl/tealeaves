...
Proof.
  ...
  change (H5) with (Map_compose (fun X => X) G1) at 1.
  change (H6) with (@pure G1 _) at 1.
  rewrite <- (Pure_compose_identity2 G1).
  change (H7) with (@mult G1 _) at 1.
...
Section with_functor.
  ...
  Proof.
    intros. unfold_ops @Map_Traverse.
  ...
  (** *** Homomorphism law *)
  (******************************************************************************)
  Lemma foldMap_morphism (M1 M2 : Type) `{Monoid_Morphism M1 M2 ϕ} : forall `(f : A -> M1),
...
Section elements.
  ...
  Proof.
    ...
    rewrite (foldMap_to_runBatch1 T _).
    reflexivity.
  Qed.
  ...
  Proof.
    intros.
    rewrite element_to_foldMap1.
    rewrite (foldMap_to_runBatch2 T A tag).
    reflexivity.
  Qed.
...
Section respectfulness_properties.
  ...
  Proof.
    ...
    rewrite (element_to_toBatch2 T A B) in hyp.
    unfold compose in *.
  ...
  Proof.
    intros. rewrite <- (traverse_purity1 T G).
    compose near t on right.
    rewrite (TraversableFunctor.DerivedInstances.traverse_map T G).
