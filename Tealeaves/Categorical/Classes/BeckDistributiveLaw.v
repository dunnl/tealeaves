...
Section BeckDistributivelaw_composite_monad.
  ...
  Proof.
    ...
    change (id ∘ ?f) with f. rewrite (fun_map_map T).
  ...
  Proof.
    ...
    do 2 reassociate ->.
    #[local] Set Keyed Unification.
    rewrite 2(fun_map_map T).
    #[local] Unset Keyed Unification.
    rewrite <- (fun_map_map U).
    ...
    rewrite <-(fun_map_map T). reassociate <-.
